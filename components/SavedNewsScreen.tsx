...
export const SavedNewsScreen: React.FC<SavedNewsScreenProps> = ({ savedArticles, onUnsave }) => {
  const handleShare = async (article: NewsArticle) => {
    ...
    if (navigator.share) {
      await navigator.share({ title: article.headline, text });
    } else {
  ...
  if (savedArticles.length === 0) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center text-neutral-500 p-4">
        <BookmarkSquareIcon className="w-16 h-16 mb-4 text-primary-600" />
      ...
      {savedArticles.map(article => (
        <div key={article.id} className="bg-neutral-800 p-4 rounded-lg shadow-md border border-neutral-700">
          ...
          <div className="flex justify-between items-center text-xs text-neutral-400 mb-3">
            ...
            <div>
              {article.ticker && (
                <span className="bg-neutral-700 text-neutral-200 px-2 py-0.5 rounded-full text-xs font-mono mr-2">
                  {article.ticker}
                </span>
              )}
              {article.category && (
                <span className="bg-primary-700 text-primary-100 px-2 py-0.5 rounded-full text-xs">
